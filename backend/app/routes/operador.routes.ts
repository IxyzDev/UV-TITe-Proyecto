...
import * as operadorControllers from '../controllers/operadores_controllers/operador.controllers'
...
router.get('/get', async (_req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ msg: 'Error al mostrar los Operador: ' + error.message});
  }
})
...
router.post('/post', async (req: Request, res: Response) => {
  try {
    const newOperadorEntry = await operadorControllers.postOperador({...req.body});
    ...
    console.log(error)
    return res.status(500).json({ msg: 'Error al crear un operador: ' + error.message });
  }
})
...
router.delete('/delete/:id', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ msg: 'Error al eliminar el Operador: ' + error.message });
  }
});
