...
router.get('/get', async (_req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ msg: 'Error al mostrar los Asignacion: ' + error.message});
  }
})
...
router.post('/post', async (req: Request, res: Response) => {
  try {
    ...
    const record = await db.Asignacion.create(newAsignacionEntry);
...
router.delete('/delete/:id', async (req: Request, res: Response) => {
  try {
    await asignacionControllers.deleteAsignacion({ ubicacion_ID: req.params.id });
    
    return res.json({ msg: 'Asignacion eliminado correctamente' });
  } catch (error: any) {
    return res.status(500).json({ msg: 'Error al eliminar el Asignacion: ' + error.message });
  }
});
