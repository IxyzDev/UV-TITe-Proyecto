...
import * as movilControllers from '../controllers/movil_controllers/movil.controllers';
...
router.get('/get', async (_req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ msg: 'Error al mostrar los Movils: ' + error.message});
  }
})
...
router.put('/put/:id', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ msg: 'Error al actualizar el Movil: ' + error.message });
  }
});
...
router.delete('/delete/:id', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ msg: 'Error al eliminar el Movil: ' + error.message });
  }
});
