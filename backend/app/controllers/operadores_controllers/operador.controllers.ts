...
export const postOperador = async (object: any): Promise<OperadoresInterface> => {
  console.log(object)
  const comprobarFuncionario = await Funcionario.findOne({where: {funcionario_ID: object.funcionario_ID}});
...
export const getOperadorById = async (object: any): Promise<OperadoresInterface> => {
...
export const deleteOperador = async (object: any): Promise<void> => {
  const result = await Operador.destroy({where: {operador_ID: object.operador_ID}});
  if (!result) {
