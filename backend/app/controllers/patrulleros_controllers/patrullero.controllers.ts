...
export const postPatrullero = async (object: any): Promise<PatrullerosInterface> => {
...
export const getPatrulleroById = async (object: any): Promise<PatrullerosInterface> => {
...
export const deletePatrullero = async (object: any): Promise<void> => {
  const result = await Patrullero.destroy({where: {patrullero_ID: object.patrullero_ID}});
  if (!result) {
