...
export const createSubSector = async (req: Request, res: Response): Promise<void> => {
  try {
    const { subsector_ID, nombre_subsector, sector_ID } = req.body;

    // Valida los datos de entrada
    if (!subsector_ID || !nombre_subsector || !sector_ID) {
      res.status(400).json({ success: false, message: 'Datos de subsector incompletos' });
      return;
    }
...
export const getAllSubSectores = async (_req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.error(error);
    res.status(500).json({ success: false, message: 'Error al obtener los subsectores' });
  }
};

// Controlador para obtener un subsector por ID
export const getSubSectorById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!subsector) {
      res.status(404).json({ success: false, message: 'Subsector no encontrado' });
      return;
    }
    ...
    console.error(error);
    res.status(500).json({ success: false, message: 'Error al obtener el subsector' });
  }
};

// Controlador para actualizar un subsector por ID
export const updateSubSector = async (req: Request, res: Response): Promise<void> => {
  try {
    const { subsector_ID } = req.params;
    ...
    res.status(200).json({ success: true, message: 'Subsector actualizado con éxito', data: subsector });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: 'Error al actualizar el subsector' });
  }
};

// Controlador para eliminar un subsector por ID
export const deleteSubSector = async (req: Request, res: Response): Promise<void> => {
  try {
    const { subsector_ID } = req.params;
    ...
    res.status(200).json({ success: true, message: 'Subsector eliminado con éxito' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: 'Error al eliminar el subsector' });
  }
};
