...
export const createFuncionario = async (object: any): Promise<FuncionariosInterface> => {
  const newFuncionarioEntry: FuncionariosInterface = {
    ...
    apellido_funcionario: verif.parseApellidoFuncionario(object.apellido_funcionario),
...
export const readFuncionarioById = async (object: any): Promise<FuncionariosInterface[]> => {
  const funcionarios = await funcionario.findOne({where: {funcionario_ID: object}});
  return funcionarios;
};
...
export const updateFuncionario = async (object: any): Promise<void> => {
  ...
  try {
    existingFuncionario = await funcionario.findOne({ where: { funcionario_ID: object.funcionario_ID } });
  ...
  try {
    ...
    apellidoVerificado = verif.parseApellidoFuncionario(object.apellido_funcionario);
    tipoVerificado = verif.parseTipoFuncionario(object.tipo_funcionario);
  } catch (error: any) {
    throw new Error('Error al verificar los campos del funcionario: ' + error.message);
  }
...
export const deleteFuncionario = async (object: any): Promise<void> => {
  try {
    const result = await funcionario.destroy({ where: { funcionario_ID: object.funcionario_ID } });
