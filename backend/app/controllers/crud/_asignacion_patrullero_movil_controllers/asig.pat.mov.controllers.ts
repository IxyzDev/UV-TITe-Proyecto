...
export const getAsignaciones = async (): Promise<
  AsignacionPatrulleroMovilInterface[]
> => {
...
export const getAsignacionById = async (object: any): Promise<void> => {
  const asignacion = await Asignacion.findByPk({
    where: { asignacion_ID: object.asignacion_ID },
  });
  if (!asignacion) {
...
export const deleteAsignacion = async (object: any): Promise<void> => {
  ...
  const result = await Asignacion.destroy({
    where: { asignacion_movil_ID: object.asignacion_movil_ID },
  });
  if (!result) {
    throw new Error("Asignacion no encontrada");
  }
